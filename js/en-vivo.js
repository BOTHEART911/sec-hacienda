/* ============================================================
   ...
   Ruta del nodo
     Se intenta /meta. Si las reglas de la base no lo permiten, cae solo
     a /chats/__envivo (que hoy sí está permitido, es donde vive el chat).
     Ver el LEEME: con 4 líneas de reglas queda en /meta, que es lo limpio.
...
(function () {
  ...
  var RUTA_PREFERIDA = 'meta';
  var RUTA_RESPALDO  = 'chats/__envivo';
  ...
  function encenderRespaldo_(motivo) {
    ...
    if (existe(timers.startPendientesAutoRefresh_)) {
      var id = vistaActiva_();
      if (id === 'view-lista') timers.startPendientesAutoRefresh_();
      if (id === 'view-asignaciones' && existe(timers.startAsignacionesAutoRefresh_)) {
        timers.startAsignacionesAutoRefresh_();
      }
    }
  }
  ...
  function arrancar_() {
    ...
    window.initFirebase_().then(function (base) {
      ...
      db = base;
      return probar_(RUTA_PREFERIDA).then(function (ok) {
        if (ok) return RUTA_PREFERIDA;
        return probar_(RUTA_RESPALDO).then(function (ok2) { return ok2 ? RUTA_RESPALDO : ''; });
      }).then(function (elegida) {
