...
function procSincronizarFiltros_() {
  ...
  PROC_FILTROS.forEach(cfg => {
    ...
    if (!panel) return;
    const firma = ops.map(o => o.valor).join('|');
...
document.getElementById('btn-expediente-guardar')?.addEventListener('click', async () => {
  ...
  const esArchivo = esArchivo_();
  const prefijo   = getPrefijo_(currentUser?.nombre || '');
  ...
  try {
    ...
    // Aviso a quien lleve ARCHIVO, salvo que sea quien está guardando
    if (!esArchivo) {
      const tels = telsArchivo_();
      for (let i = 0; i < tels.length; i++) {
        const msg =
          `Tienes una solicitud de *${currentUser?.nombre || ''}*\n` +
          `*N° Exp. Interno:* ${row.expediente || row.id_proceso}\n` +
          `> Revisa la App`;
        sendProcWA_(tels[i], msg);
      }
    }
