...
(function () {
  ...
  var listaUsuarios = [];       // usuarios activos para elegir
  var uidElegido = '';
  var pinBuf = '';
  ...
  function conRoles_() { return !!(perfil && perfil.roles.length); }

  /* ============================================================
     PERMISOS POR ROL
     Si el usuario todavía no tiene roles en la hoja, se respeta
     exactamente lo que decidía la app antes (respaldo).
     ============================================================ */
  function envolver_(nombreFn, fnRol) {
    var original = window[nombreFn];
    window[nombreFn] = function () {
      if (!currentUser) return false;
      if (!conRoles_()) return typeof original === 'function' ? original.apply(this, arguments) : false;
      return fnRol();
    };
  }

  envolver_('canSeeAgregarSolicitud_', function () { return tieneRol_('ATENCION', 'ADMIN', 'DEV'); });
  envolver_('canSeePendientes_',       function () { return tieneRol_('ATENCION', 'ADMIN', 'DEV'); });
  envolver_('canSeeAtendidasChat_',    function () { return tieneRol_('ADMIN', 'DEV'); });
  envolver_('canSeeBDPredial_',        function () { return tieneRol_('ABOGADO', 'ASISTENTE', 'ADMIN', 'DEV'); });
  envolver_('canSeeAgregarAsignacion_',function () { return tieneRol_('ADMIN', 'DEV'); });

  function puedeVerSemaforo_() {
    if (!conRoles_()) return null; // null = que decida el código viejo
    return tieneRol_('ABOGADO', 'ASISTENTE', 'ARCHIVO', 'ADMIN', 'DEV');
  }
  ...
  function pintarCabecera_() {
    ...
    var bSema = document.getElementById('btn-semaforo');
    var ver = puedeVerSemaforo_();
    if (bSema && ver !== null) bSema.style.display = ver ? '' : 'none';
  }
  ...
  function marcaLogin_() {
    ...
    if (!document.getElementById('idn-login')) {
      tab.innerHTML =
        '<div id="idn-login">' +
          '<div id="idn-paso-usuario">' +
            '<p class="idn-titulo">Elige tu usuario</p>' +
            '<input id="idn-buscar" type="text" autocomplete="off" placeholder="Buscar por nombre" />' +
            '<div id="idn-usuarios" class="idn-usuarios"></div>' +
          '</div>' +
      ...
      document.getElementById('idn-buscar').addEventListener('input', pintarUsuarios_);
  ...
  function pintarUsuarios_() {
    var cont = document.getElementById('idn-usuarios');
    if (!cont) return;
    var q = (document.getElementById('idn-buscar').value || '').trim().toUpperCase();

    var cuentas = leer_(K_CUENTAS) || [];
    var recientes = cuentas.map(function (c) { return c.uid; });

    var lista = listaUsuarios.slice().sort(function (a, b) {
      var ra = recientes.indexOf(a.uid), rb = recientes.indexOf(b.uid);
      if (ra !== -1 || rb !== -1) {
        if (ra === -1) return 1;
        if (rb === -1) return -1;
        return ra - rb;
    ...
    if (q) lista = lista.filter(function (u) { return u.nombre.indexOf(q) !== -1; });

    cont.innerHTML = '';
    if (!lista.length) {
      cont.innerHTML = '<p class="idn-vacio">Sin usuarios para mostrar</p>';
      return;
    }
    ...
    lista.forEach(function (u) {
      var b = document.createElement('button');
      b.type = 'button';
      b.className = 'idn-usuario' + (u.tienePin === false ? ' sinpin' : '');
      b.innerHTML =
        '<span class="idn-mini">' +
      ...
      b.addEventListener('click', function () {
        if (u.tienePin === false) {
          Swal.fire({ icon: 'info', title: 'Sin PIN', text: 'Este usuario aún no tiene PIN. Entra con Documento.' });
          return;
        }
        uidElegido = u.uid;
        pinBuf = '';
  ...
  function pintarPaso_() {
    ...
    var u = listaUsuarios.filter(function (x) { return x.uid === uidElegido; })[0];
  ...
  async function cargarUsuarios_(forzar) {
    var cache = leer_(K_LISTA);
    if (!forzar && cache && Array.isArray(cache.lista) && cache.lista.length) {
      listaUsuarios = cache.lista;
      pintarPaso_();
    }
    try {
      var lista = await apiGet('usuarioslogin', {});
      if (Array.isArray(lista)) {
        listaUsuarios = lista;
        escribir_(K_LISTA, { lista: lista, ts: Date.now() });
        pintarPaso_();
      }
    } catch (_) {
      if (!listaUsuarios.length) {
        var c = document.getElementById('idn-usuarios');
        if (c) c.innerHTML = '<p class="idn-vacio">No se pudo cargar la lista. Entra con Documento.</p>';
      }
    }
  }
  ...
  async function entrarConPin_() {
    try {
      ...
      if (data && data.inactivo) {
        ...
        uidElegido = ''; pintarPaso_(); cargarUsuarios_(true);
        return;
      }
      if (data && data.sinPin) {
  ...
  window.IDN = {
    ...
    recargarUsuarios: function () { return cargarUsuarios_(true); },
